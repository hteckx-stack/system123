    ...
    <div className="flex flex-col h-[calc(100vh-8rem)] gap-3 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-1">
        <div>
          ...
          <p className="text-[#6B7280] text-xs">Centralized messaging, broadcasts, and official documents.</p>
        </div>
      </div>
      ...
      <Tabs defaultValue="messages" className="flex-1 flex flex-col overflow-hidden">
        <TabsList className="bg-white p-1 rounded-xl shadow-soft border border-slate-100 self-start mb-2">
          <TabsTrigger value="messages" className="rounded-lg font-bold px-6 h-9">Direct Messages</TabsTrigger>
          <TabsTrigger value="broadcasts" className="rounded-lg font-bold px-6 h-9">Broadcasts</TabsTrigger>
          <TabsTrigger value="documents" className="rounded-lg font-bold px-6 h-9">Files & Documents</TabsTrigger>
        </TabsList>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 mb-2">
             <div className="flex items-center gap-3">
              <Select value={topicFilter} onValueChange={setTopicFilter}>
                <SelectTrigger className="w-[180px] border-none shadow-soft bg-white h-9 rounded-xl text-xs">
              ...
              <CardHeader className="bg-white border-b p-3">
                <div className="relative">
                  ...
                  <Input 
                    ...
                    className="pl-9 bg-slate-50 border-none rounded-xl h-9 text-xs"
                    value={searchQuery}
                  ...
                  {convsLoading ? (
                    ...
                    conversations.map((conv) => (
                      <div
                        ...
                        className={cn(
                          "flex flex-col gap-1 p-3.5 rounded-2xl cursor-pointer transition-all",
                          selectedConvId === conv.id ? "bg-primary/5 shadow-inner" : "hover:bg-slate-50"
                        )}
                      >
                        <div className="flex items-center justify-between mb-0.5">
                          <span className="font-bold text-slate-900 text-[13px]">{conv.staff_name}</span>
              ...
              {selectedConv ? (
                <>
                  <CardHeader className="bg-slate-50/50 border-b p-3 px-5 flex flex-row items-center gap-4">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="h-8 w-8 rounded-xl bg-primary flex items-center justify-center font-bold text-white shadow-lg shadow-primary/20 text-xs">
                        {selectedConv.staff_name.charAt(0)}
                      </div>
                      <div>
                        <h3 className="font-bold text-slate-900 text-[13px]">{selectedConv.staff_name}</h3>
                    ...
                    <div className="space-y-4">
                      {messagesLoading ? (
                        <div className="flex flex-col gap-4">
                          <Skeleton className="h-9 w-1/3 rounded-2xl rounded-tl-none" />
                          <Skeleton className="h-7 w-1/4 self-end rounded-2xl rounded-tr-none" />
                        </div>
                      ) : (
                        messages.map((msg) => (
                          <div
                  ...
                  <div className="p-3 border-t">
                    <form onSubmit={handleSendMessage} className="flex gap-2">
                      <Input
                        ...
                        className="flex-1 h-10 rounded-xl bg-slate-50 border-none text-xs"
                      />
        ...
        <TabsContent value="broadcasts" className="flex-1 overflow-hidden m-0">
          <div className="grid gap-3 lg:grid-cols-12 h-full">
            <Card className="lg:col-span-5 border-none shadow-soft rounded-3xl overflow-hidden bg-white flex flex-col">
              ...
              <ScrollArea className="flex-1">
                <CardContent className="space-y-3 pt-4 px-6">
                  ...
                  <div className="space-y-1">
                    ...
                    <Textarea 
                      ...
                      className="min-h-[140px] rounded-2xl bg-slate-50 border-slate-200 p-4 text-xs leading-relaxed"
              ...
              <CardFooter className="bg-slate-50 border-t p-3 px-6">
                <Button 
            ...
            <div className="lg:col-span-7 flex flex-col overflow-hidden">
              <h2 className="text-base font-bold text-[#1A1A1A] flex items-center gap-2 mb-2">
                  ...
                  {broadcastsLoading ? (
                    ...
                    <Card key={ann.id} className="border-none shadow-soft rounded-2xl bg-white overflow-hidden border-l-4 border-l-[#0D47A1]">
                      <CardHeader className="p-3 px-4 pb-0">
                        <div className="flex justify-between items-center text-[8px] text-slate-400 font-bold uppercase tracking-wider mb-1">
                        ...
                        <CardTitle className="text-[13px] font-bold text-[#1A1A1A]">{ann.title}</CardTitle>
                      </CardHeader>
                      <CardContent className="p-3 px-4 pt-1 pb-3">
                        <p className="text-[11px] text-[#6B7280] line-clamp-2 leading-relaxed">{ann.message}</p>
                      </CardContent>
                    </Card>
                  ))}
                </div>
        ...
        <TabsContent value="documents" className="flex-1 overflow-hidden m-0">
          <div className="grid gap-3 lg:grid-cols-12 h-full overflow-hidden">
            <div className="lg:col-span-4 flex flex-col gap-3 overflow-hidden">
              <Card className="border-none shadow-soft rounded-3xl bg-white overflow-hidden flex flex-col">
                <CardHeader className="bg-primary p-3 px-5 text-white">
                  <div className="flex items-center gap-3">
                    <Plus className="h-4 w-4" />
                    <CardTitle className="text-base">Create & Send</CardTitle>
                  </div>
                </CardHeader>
                <ScrollArea className="flex-1">
                    ...
                    <Tabs defaultValue="upload">
                      ...
                      <TabsContent value="upload" className="m-0 space-y-2">
                        <form onSubmit={handleUploadSubmit} className="space-y-2">
                          ...
                          <div className="space-y-1">
                            ...
                            <Input type="file" onChange={handleFileChange} className="h-9 rounded-xl border-dashed text-[11px] pt-2" />
                          </div>
                          <Button disabled={uploading} className="w-full rounded-xl h-9 font-bold text-xs">
                      ...
                      <TabsContent value="template" className="m-0 space-y-2">
                        <form onSubmit={handleGenerateSubmit} className="space-y-2">
                          ...
                          <Button disabled={generating} className="w-full rounded-xl h-9 font-bold text-xs">
            ...
            <div className="lg:col-span-8 flex flex-col gap-3 overflow-hidden">
              <Card className="border-none shadow-soft rounded-3xl bg-white overflow-hidden flex flex-col">
                <CardHeader className="bg-slate-50 border-b flex flex-row items-center justify-between p-3 px-5">
                  <div className="flex items-center gap-3">
                  ...
                  <Table>
                    <TableHeader className="bg-slate-50/50">
                      <TableRow className="h-9 uppercase text-[8px] tracking-widest font-bold">
                        <TableHead className="px-5">Recipient</TableHead>
                    ...
                    <TableBody>
                      {documentsLoading ? (
                        Array.from({ length: 5 }).map((_, i) => (
                          <TableRow key={i}>
                            <TableCell colSpan={5} className="p-3"><Skeleton className="h-5 w-full" /></TableCell>
                          </TableRow>
                        ))
                      ) : documents && documents.length > 0 ? (
                        documents.map((doc) => (
                          <TableRow key={doc.id} className="h-12 hover:bg-slate-50/50">
                            <TableCell className="px-5 font-bold text-slate-900 text-[11px]">{doc.staffName}</TableCell>
                            <TableCell>
                              <Badge className="bg-slate-100 text-slate-600 border-none text-[8px] font-bold">
                                {doc.type}
                              </Badge>
                            </TableCell>
                            <TableCell className="text-[9px] text-slate-500 max-w-[120px] truncate">{doc.fileName}</TableCell>
                            <TableCell className="text-[9px] font-medium text-slate-400">{doc.date}</TableCell>
                        ...
                        <TableRow>
                          <TableCell colSpan={5} className="h-24 text-center text-slate-300 text-[11px]">
              ...
              <Card className="border-none shadow-soft rounded-3xl bg-white overflow-hidden">
                <CardHeader className="bg-slate-50 border-b p-3 px-5">
                  <CardTitle className="text-base font-bold">System Templates</CardTitle>
                </CardHeader>
                <CardContent className="p-0">
                  <Accordion type="single" collapsible className="w-full">
                    <AccordionItem value="contract" className="border-none">
                      <AccordionTrigger className="px-5 py-2.5 hover:no-underline font-bold text-[12px]">Contract Template</AccordionTrigger>
                      <AccordionContent className="px-5 pb-3">
                        <Textarea value={contractTemplate} onChange={(e) => setContractTemplate(e.target.value)} className="min-h-[100px] text-[9px] font-mono mb-2" />
                        <Button size="sm" onClick={() => toast({ title: "Template Saved" })} className="rounded-lg h-7 text-[10px]">Update Template</Button>
                      </AccordionContent>
                    </AccordionItem>
                    <AccordionItem value="payslip" className="border-none border-t">
                      <AccordionTrigger className="px-5 py-2.5 hover:no-underline font-bold text-[12px]">Payslip Template</AccordionTrigger>
                      <AccordionContent className="px-5 pb-3">
                        <Textarea value={payslipTemplate} onChange={(e) => setPayslipTemplate(e.target.value)} className="min-h-[100px] text-[9px] font-mono mb-2" />
                        <Button size="sm" onClick={() => toast({ title: "Template Saved" })} className="rounded-lg h-7 text-[10px]">Update Template</Button>
                      </AccordionContent>
                    </AccordionItem>
                  </Accordion>
